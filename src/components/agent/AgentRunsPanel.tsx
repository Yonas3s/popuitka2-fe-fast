...
export const AgentRunsPanel = ({ projectId, compact = false }: AgentRunsPanelProps) => {
  ...
  const onRun = async () => {
    ...
    try {
      ...
      pushToast('Не удалось запустить агента', 'error');
    }
  };
  ...
  const runDisabled = creatingRun || loadingRuns || !prompt.trim();
